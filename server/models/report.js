...
reports.push(
	new Report({
		...
		query: {
			...
			chart: 'SELECT name as row_label, to_char(dd, \'MM/DD/YYYY\') as col_label, ROUND(COALESCE(SUM(total),0) / 100.0, 2)::money as "value" FROM generate_series( date_trunc(\'day\', $2::timestamp ), date_trunc(\'day\', $3::timestamp ), \'1 day\'::interval) dd CROSS JOIN channel_partners LEFT JOIN transaction_items ON transaction_items.src_created_at BETWEEN dd AND dd + \'1 day\'::interval AND transaction_items.channel_partner_id IS NOT NULL AND transaction_items.channel_partner_id = channel_partners.id WHERE channel_partners.parent_id = $1 GROUP BY channel_partners.id, dd ORDER BY channel_partners.id, dd',
